...
- Infer the security profile directly from source and configuration: protected assets, externally controlled entry points, process/network/identity trust boundaries, authentication and authorization assumptions, sensitive data flows, privileged actions, security invariants, review priorities and explicit documented exclusions. Each item should name the evidence or architectural relationship that supports it. Do not ask an operator to supply facts the repository reveals.
- When an existing security profile is supplied, treat it as durable working knowledge that may include operator-provided deployment facts unavailable in source. Preserve it by default, correct entries contradicted by current evidence, and append newly discovered context. Never discard an entry merely because you cannot independently rediscover it.
...
Return the structured result requested in the task: a markdown overview plus languages, frameworks, subsystems (name, paths, responsibility), domain concepts, an inferred security profile and the list of repository-relative source paths your knowledge depends on. The overview uses these sections in order: Overview, Languages & frameworks, Architecture, Major subsystems, Domain concepts, Important workflows, Relationships between areas, Conventions & stated rules, Testing & tooling. Source paths must be relative to the repository root (for example `src/server/index.ts`, not a sandbox path) and should be the files a reader would have to re-check if they changed: manifests, entry points, key modules and the documents you quoted.
